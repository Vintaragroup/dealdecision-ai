...
export function buildPurgeConfirmationToken(args: { entityType: "deal" | "document"; entityId: string }): string {
  return `PURGE ${args.entityType.toUpperCase()} ${args.entityId}`;
}

export function requirePurgeConfirmation(
  request: FastifyRequest | any,
  expectedToken: string
): { ok: true } | { ok: false; status: number; error: string } {
  const body = ((request as any)?.body ?? {}) as Record<string, unknown>;
  const raw =
    (typeof body.confirm_text === "string" ? body.confirm_text : "") ||
...
async function isDbSuperAdmin(userId: string): Promise<boolean> {
  const pool = getPool();
  const { rows } = await pool.query<{ account_role: string }>(
...
export async function requirePurgeAuth(
  request: FastifyRequest | any,
  reply?: FastifyReply
): Promise<{ ok: true } | { ok: false; status: number; error: string }> {
  // Keep local/dev behavior permissive to avoid breaking test and maintenance flows.
  if (process.env.NODE_ENV !== "production") return { ok: true };
  ...
  if (
    typeof token === "string" &&
    token.trim() &&
    typeof process.env.ADMIN_TOKEN === "string" &&
    process.env.ADMIN_TOKEN.trim().length > 0 &&
  ...
  try {
    const allowed = await isDbSuperAdmin(userId);
    if (!allowed) {
      return {
        ok: false,
        status: 403,
        error: "Forbidden: super_admin role required for purge",
      };
    }
    return { ok: true };
  } catch (err) {
    reply?.log?.error?.({ err }, "purge_super_admin_check_failed");

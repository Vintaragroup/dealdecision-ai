...
export function buildNarrationPrompt(args: BuildNarrationPromptArgs): { system: string; user: string } {
	...
	const system =
		...
		" (3g) KPI-token rule (sections+insights): Do NOT use KPI tokens (revenue/arr/mrr/irr/moic/cap_rate/cac/ltv/margin/runway/burn) in sections[*].body or insights[*].claim unless you include at least one citation from CITATION_CATALOG_JSON that matches the KPI source. If you do not have a matching KPI citation, rewrite using non-KPI phrasing and avoid numbers. Use these substitutions when rewriting: burn -> \"cash outflow rate\" or \"monthly spend rate\"; runway -> \"time-to-cash-constraint\"; revenue -> \"top-line performance\". " +

...
export async function runExtractVisualsCoordinator(job: Job) {
	...
	if (docsReady === 0) {
		const waitOutcome = await waitForIngest({
			pool,
			job,
			candidateDocumentIds,
			initialBlockedDocs: blockedDocs,
			initialBlockedReasonsCount: blockedReasonsCount,
			docsTotal,
			dealId: dealId ?? null,
			documentsMetaStatusOk,
			originalFileTablesOk,
		});
		if (waitOutcome.timedOut) return waitOutcome.result;
		readyDocumentIds = waitOutcome.readyDocumentIds;
		docsReady = waitOutcome.docsReady;
		docsBlocked = waitOutcome.docsBlocked;
		docsBlockedPending = waitOutcome.docsBlockedPending;
		targetDocumentIds = readyDocumentIds;
	}

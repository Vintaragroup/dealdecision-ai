...
import { buildUiPreviewV1 } from "../src/lib/ui-preview-v1";
...
function palmLikeDpuRowsWithFinancialTableAndTiers(docId: string): Array<{ document_id: string; page_index: number; payload: any }> {
	const pages: Array<{ page_index: number; title: string; bullets: string[] }> = [
		...
		{ page_index: 2, title: "Product", bullets: ["Premium golf apparel and accessories including gloves and hats."] },
		{ page_index: 4, title: "Market", bullets: ["ICP: core 18–34 male golfers, expanding into women and youth. Sells via DTC and wholesale to green grass courses, retailers, and pro shops."] },
		{ page_index: 5, title: "Industry outlook", bullets: ["Golf participation is growing, supporting premium apparel demand."] },
		{ page_index: 6, title: "Traction", bullets: ["Wholesale accounts are growing alongside DTC conversion and repeat customer behavior."] },
		// Channel-attributed revenue should remain available but must not win as canonical.
		...
		// Financial table row includes year columns; should become canonical revenue (2024) instead of $800k.
		{ page_index: 12, title: "The Financials", bullets: ["Revenue 2022 $1.234M 2023 $1.900M 2024 $2.476M 2025 $2.900M 2026 $3.400M"] },
	];
...
test("ui_preview_v1 uses canonical revenue + tier hero summary (Palm)", async () => {
	const dealId = "00000000-0000-4000-8000-00000000ca13";
	const dioId = "dio-palm-ui-preview";
	...
	const pool = mkMockPool({
		...
		dioId,
		dio,
		dpuRows: palmLikeDpuRowsWithFinancialTableAndTiers("doc-palm"),
	});
	...
	const revenueValue = String(dto?.header_tiles?.revenue?.value ?? "");
	assert.ok(revenueValue.length > 0, "expected ui_preview_v1 header_tiles.revenue.value");
	assert.doesNotMatch(revenueValue, /800\s*,?\s*000|\$\s*800\s*k|800\s*k/i);

	// Debug hook: selection_reason should be attached when present.
	const sel = String(dto?.header_tiles?.revenue?.selection_reason ?? "");
	assert.ok(sel.length > 0, "expected ui_preview_v1 header_tiles.revenue.selection_reason");

	const dealSummary = String(dto?.summaries?.deal_summary?.value ?? "");
	assert.ok(dealSummary.length > 0, "expected ui_preview_v1 summaries.deal_summary");
	assert.match(dealSummary, /golf apparel and accessories/i);
	assert.match(dealSummary, /\bDTC\b/i);
	assert.match(dealSummary, /wholesale/i);
});

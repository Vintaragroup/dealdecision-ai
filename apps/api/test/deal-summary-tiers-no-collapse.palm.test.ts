    ...
    query: async (sql: string, params?: unknown[]) => {
      const q = String(sql);
        ...
        assert.deepEqual(params, [args.dealId]);
          return { rows: [{ id: args.dealId, llm_phase_mode: "exploratory" }] };
      }
...
function palmLikeDpuRowsForTiers(docId: string): Array<{ document_id: string; page_index: number; payload: any }> {
...
test("deal summary tiers should not collapse (Palm-like)", async () => {
  const dealId = "00000000-0000-4000-8000-00000000ca12";
  const dioId = "dio-palm-tiers";
  ...
  const pool = mkMockPool({
    ...
    dioId,
    dio,
    dpuRows: palmLikeDpuRowsForTiers("doc-palm"),
  });
  ...
  const tiers = body?.deal_summary?.tiers ?? body?.report?.deal_summary?.tiers;
  assert.ok(tiers, "Expected deal_summary.tiers");

  const hero = typeof tiers.hero === "string" ? tiers.hero.trim() : "";
  const overview = typeof tiers.overview === "string" ? tiers.overview.trim() : "";
  const deep = typeof tiers.deep === "string" ? tiers.deep.trim() : "";

  assert.ok(hero.length > 0, "Expected non-empty hero tier");
  assert.ok(overview.length > 0, "Expected non-empty overview tier");
  assert.ok(deep.length > 0, "Expected non-empty deep tier");

  assert.notEqual(hero, overview, "Tier collapse: hero == overview");
  assert.notEqual(overview, deep, "Tier collapse: overview == deep");

  // Soft sanity: the tier ordering should generally increase in detail.
  assert.ok(overview.length > hero.length, "Expected overview longer than hero");
  assert.ok(deep.length > overview.length, "Expected deep longer than overview");
});

 ...
 *
 * Pure parsing utilities for extracting structured data from
 * InvestorInsightsRenderPackage sections.
 *
...
export function findSectionBody(rp: RenderPackage, key: string): string | null {
...
export function findSectionItems(rp: RenderPackage, key: string): unknown[] {
  const sec = rp.sections.find((s) => s.key === key);
  return Array.isArray(sec?.items) ? sec.items : [];
}

/** Return true when a section key exists in the render package sections. */
export function hasSectionKey(rp: RenderPackage, key: string): boolean {
...
import type { StageLabel } from "./types.js";

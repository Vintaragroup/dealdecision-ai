...
const NEXT_ACTION_NORMALIZATION_RULES: Array<[RegExp, string]> = [
  ...
  [/reconcile.*contradict|contradict.*reconcile|document.*contradict|contradict.*document|deal model.*cannot be relied|conflicting.*claim|resolve.*claim|contradict.*claim|document.level contradiction|document-level contradiction/i,
    'Reconcile contradicting claims across submitted documents before relying on any financial figures.'],
  [/consolidated.*data room|re.submit.*data room|data room.*consistent|re.submit with verified financial|re-submit with verified financial/i,
    'Re-submit a consolidated data room with consistent, verified figures across all documents.'],

import type { AuthoritativeSummaryEvidenceRefV1 } from './selectAuthoritativeProductSummaryV1';

export type AuthoritativeMarketSummarySelectionV1 = {
  value: string | null;
  confidence: number | null;
  sources: AuthoritativeSummaryEvidenceRefV1[];
  source: 'report.structured_summary.market_summary_v1' | 'missing';
};
...
export function selectAuthoritativeMarketSummaryV1(report?: unknown | null): AuthoritativeMarketSummarySelectionV1 {
  const r: any = report as any;
  ...
  const block = structured?.market_summary_v1;
  ...
  return {
    ...
    confidence,
    sources,
    source: value ? 'report.structured_summary.market_summary_v1' : 'missing',
  };
}

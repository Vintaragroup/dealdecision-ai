...
**Generated:** 2026-04-08T23:55:17.218721+00:00  
**Overall status:** ✅ PASS
...
| 2 | Albuquerque | `267c4979` | 12 | 7 | 0 | 2 | 3 | ✅ PASS |
| 3 | Allurion | `a85b0ac0` | 11 | 9 | 0 | 0 | 2 | ✅ PASS |
| 4 | Cinco | `0fcec035` | 0 | 0 | 0 | 0 | 0 | ✅ PASS |
| 5 | Deal Decision | `517be946` | 5 | 4 | 0 | 1 | 0 | ✅ PASS |
| 6 | MagarianFund | `0e8fa8ae` | 17 | 14 | 0 | 1 | 2 | ✅ PASS |
| 7 | Palm | `c4f10092` | 0 | 0 | 0 | 0 | 0 | ✅ PASS |
| 8 | Probility | `42be8b30` | 11 | 9 | 0 | 0 | 2 | ✅ PASS |
| 9 | Qredible | `b21b894e` | 6 | 6 | 0 | 0 | 0 | ✅ PASS |
| 10 | StackFactor | `adb2a1cf` | 8 | 6 | 0 | 2 | 0 | ✅ PASS |
...
- **Status:** ✅ PASS
- **Checks:** 12 total — 7 PASS, 0 FAIL, 2 KNOWN_ISSUE, 3 SKIP

**Extraction checks**

| Outcome | Category | Label | Expected | Actual |
|---------|----------|-------|----------|--------|
| ✅ PASS | `extraction` | extraction:raise_amount | `raise_amount ≈ 11868000 (±5.0%)` | Decimal('11900000') |

**Noise checks**

| Outcome | Category | Label | Expected | Actual |
|---------|----------|-------|----------|--------|
| 🟡 KNOWN_ISSUE `[BUG-RE-3]` | `noise` | Revenue must not be >= $1T (construction budget sales tax noise) | `count == 0` | 1 — `actual count=1, expected=0` |
| 🟡 KNOWN_ISSUE `[BUG-RE-3]` | `noise` | financial_breakdown revenue must not be >= $1B | `<= 999999999` | 2352769000000000.0 — `actual=2352769000000000.0, max=999999999` |
...
| ⚠️ SKIP | `dio` | deal_overview_v2 business_model must be Real estate structured investment | `unknown assertion type` | <object object at 0x102368ea0> |

**Evidence checks**

| Outcome | Category | Label | Expected | Actual |
|---------|----------|-------|----------|--------|
| ⚠️ SKIP | `evidence` | real_estate archetype evidence must include re:real_estate rule | `error` | None |
| ⚠️ SKIP | `evidence` | real_estate archetype evidence must include re:noicap rule | `error` | None |

**🟡 Known issues matched:**

- Revenue must not be >= $1T (construction budget sales tax noise) — `BUG-RE-3`: expected `count == 0`, got `1`
- financial_breakdown revenue must not be >= $1B — `BUG-RE-3`: expected `<= 999999999`, got `2352769000000000.0`
...
- **Status:** ✅ PASS
- **Checks:** 5 total — 4 PASS, 0 FAIL, 1 KNOWN_ISSUE, 0 SKIP

**Extraction checks**

| Outcome | Category | Label | Expected | Actual |
|---------|----------|-------|----------|--------|
| 🟡 KNOWN_ISSUE `[BUG-DD-1]` | `extraction` | extraction:revenue | `revenue ≈ 3337000 (±10.0%)` | Decimal('2028') — `actual=2028, expected=3337000, tol=10.0%` |
| ✅ PASS | `extraction` | extraction:burn_rate | `burn_rate ≈ 251536 (±15.0%)` | Decimal('251536') |
...
- extraction:revenue — `BUG-DD-1`: expected `revenue ≈ 3337000 (±10.0%)`, got `Decimal('2028')`
...
- **Status:** ✅ PASS
- **Checks:** 17 total — 14 PASS, 0 FAIL, 1 KNOWN_ISSUE, 2 SKIP

**Extraction checks**

| Outcome | Category | Label | Expected | Actual |
|---------|----------|-------|----------|--------|
| ⚠️ SKIP | `extraction` | extraction:financial_breakdown_data_quality | `financial_breakdown_data_quality = None` | None |
| ⚠️ SKIP | `extraction` | extraction:financial_coverage_has_xlsx | `financial_coverage_has_xlsx = False` | None |
...
| ✅ PASS | `noise` | Revenue value must not exist (no fund operating revenue) | `financial_breakdown_v1.current_state.revenue == None` | <object object at 0x102368ea0> |
...
| 🟡 KNOWN_ISSUE `[BUG-MFUND-3]` | `dio` | business_archetype_v1 from DIO must be null (known behavior BUG-MFUND-3) | `phase1.business_archetype_v1.value == None` | 'fund_spv' — `actual='fund_spv', expected=None` |
...
- business_archetype_v1 from DIO must be null (known behavior BUG-MFUND-3) — `BUG-MFUND-3`: expected `phase1.business_archetype_v1.value == None`, got `'fund_spv'`
...
| Outcome | Category | Label | Expected | Actual |
|---------|----------|-------|----------|--------|
| ✅ PASS | `extraction` | extraction:arr | `arr ≈ 1000000 (±20.0%)` | Decimal('1000000') |
| ⚠️ SKIP | `extraction` | extraction:raise_amount | `raise_amount = 4000000` | None |
...
| Outcome | Category | Label | Expected | Actual |
|---------|----------|-------|----------|--------|
| ⚠️ SKIP | `evidence` | capital_logic raise must be present | `no diagnostic assertion` | True |
...
| Outcome | Category | Label | Expected | Actual |
|---------|----------|-------|----------|--------|
| ✅ PASS | `extraction` | extraction:arr | `arr ≈ 1582164 (±20.0%)` | Decimal('1582164') |
| ✅ PASS | `extraction` | extraction:mrr | `mrr ≈ 131847 (±20.0%)` | Decimal('131847') |
...
| Outcome | Category | Label | Expected | Actual |
|---------|----------|-------|----------|--------|
| ✅ PASS | `report` | current_state.burn_rate should be null | `financial_breakdown_v1.current_state.burn_rate == None` | <object object at 0x102368ea0> |
...
- **Status:** ✅ PASS
- **Checks:** 8 total — 6 PASS, 0 FAIL, 2 KNOWN_ISSUE, 0 SKIP

**Extraction checks**

| Outcome | Category | Label | Expected | Actual |
|---------|----------|-------|----------|--------|
| 🟡 KNOWN_ISSUE `[BUG-SF-1]` | `extraction` | extraction:revenue | `revenue ≈ 23000 from source_kinds=['kpi_tile']` | '3 (source_kind=xlsx)' — `source_kind='xlsx' not in allowed=['kpi_tile']` |
| 🟡 KNOWN_ISSUE `[BUG-SF-1]` | `extraction` | extraction:cash | `cash ≈ 44000 from source_kinds=['kpi_tile']` | '7812000 (source_kind=xlsx)' — `source_kind='xlsx' not in allowed=['kpi_tile']` |
...
- extraction:revenue — `BUG-SF-1`: expected `revenue ≈ 23000 from source_kinds=['kpi_tile']`, got `'3 (source_kind=xlsx)'`
- extraction:cash — `BUG-SF-1`: expected `cash ≈ 44000 from source_kinds=['kpi_tile']`, got `'7812000 (source_kind=xlsx)'`
...
| Outcome | Category | Label | Expected | Actual |
|---------|----------|-------|----------|--------|
| ✅ PASS | `extraction` | extraction:revenue | `revenue ≈ 8000 (±15.0%)` | Decimal('8000') |
| ✅ PASS | `extraction` | extraction:burn_rate | `burn_rate ≈ 32000 (±15.0%)` | Decimal('32000') |
...
- ✅ **Albuquerque** — PASS (7/12 checks) — strong release-gate candidate
- ✅ **Allurion** — PASS (9/11 checks) — strong release-gate candidate
- ⚪ **Cinco** — no checks defined — needs GT structure before release-gating
- ✅ **Deal Decision** — PASS (4/5 checks) — strong release-gate candidate
- ✅ **MagarianFund** — PASS (14/17 checks) — strong release-gate candidate
- ⚪ **Palm** — no checks defined — needs GT structure before release-gating
- ✅ **Probility** — PASS (9/11 checks) — strong release-gate candidate
- ✅ **Qredible** — PASS (6/6 checks) — strong release-gate candidate
- ✅ **StackFactor** — PASS (6/8 checks) — strong release-gate candidate

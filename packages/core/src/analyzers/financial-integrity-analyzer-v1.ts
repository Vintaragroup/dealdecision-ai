...
/**
 * Synchronous computation of FinancialIntegrityV1 from a fact array.
 *
 * Exported for callers that cannot await (e.g. the synchronous report compiler)
 * when the DIO's stored `financial_integrity_v1` is absent because the DIO
 * predates the analyzer being added to the pipeline.
 *
 * All module-level helpers (constants, `makeFlag`, private utilities) are already
 * in scope — this function uses exactly the same logic as `FinancialIntegrityAnalyzerV1.analyze()`.
 */
export function computeFinancialIntegrityV1(
  rawFacts: FinancialFactV1[] | null | undefined,
): FinancialIntegrityV1 {
  const computed_at = new Date().toISOString();
  const facts: FinancialFactV1[] = Array.isArray(rawFacts)
    ? rawFacts.filter(
...
> {
  ...
  async analyze(input: FinancialIntegrityInput): Promise<FinancialIntegrityV1> {
    return computeFinancialIntegrityV1(input.financial_facts);
  }
}

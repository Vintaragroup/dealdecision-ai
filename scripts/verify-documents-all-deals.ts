...
async function main() {
  ...
  await Promise.all(
    selectedDeals.map((deal) =>
      limit(async () => {
        ...
        try {
          const verifyUrl = new URL(`/api/v1/deals/${encodeURIComponent(deal.id)}/documents/verify`, base);
          const resp = await fetchJson<EnqueueResponse>(verifyUrl, { method: "POST", body: "{}" });
  ...
  console.log(
    `Wait complete in ${formatDuration(waitElapsed)}: succeeded=${succeeded} failed=${failed} cancelled=${cancelled} timeout=${timeouts} error=${errors}`
  );
